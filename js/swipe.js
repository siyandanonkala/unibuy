window.UniBuySwipe = (function () {
    ...
    function buildSlide(product, pos) {
        ...
        actions.innerHTML =
            ...
            actionButton("save", "fa-bookmark", "Save", (window.UniBuySaved && window.UniBuySaved.isSaved(product.id)) ? "saved" : "") +
    ...
    function handleAction(action, product, btn, toastEl) {
        if (action === "like") {
            ...
            const nowSaved = window.UniBuySaved ?
                window.UniBuySaved.toggleSaved(product.id) :
                !btn.classList.contains("saved");

            btn.classList.toggle("saved", nowSaved);
            showToast(toastEl, nowSaved ? "Saved" : "Removed from saved items");
